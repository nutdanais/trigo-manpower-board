...
- **+ New Mission**: mission number, host, customer, shift (day/night) with start/end time, engineer. The engineer's colour marks the card: full strength on its border, and a soft wash of the same colour behind the header — so you can still pick out one engineer's missions across the whole board, without the board being a wall of saturated blocks. Click a mission header to edit, delete, or hide it.
  ...
  - The **location line** (📍, under the customer/time/engineer line) is the host's address from the Host list, and it is a **link** when that host has a Google Maps URL — tap it to open the map. It was previously only in the Host List tab, which is a planner's screen: the one question the crew reading a shared board actually has ("where do I go?") was the one thing the board didn't answer. A host with only a map link and no address shows "Open in Google Maps"; a host with neither shows no line, so filling in the Host list is what makes this appear. It survives into the printed PDF as a real link (see **Export**).
...
- **Export / PDF**: two ways to take the board off the screen. Both print the same thing — the board with the capture header on, empty missions and empty leave zones dropped, the Standby / Available On-call pools folded in, and light theme forced regardless of who is exporting.
  - **📷 Export** saves a high-resolution JPG. Use it where only an image will do.
  - **🖨 PDF** opens the browser's print dialog — choose **Save as PDF**. Prefer this for sharing with the crew. It is *vector*: the text is drawn from outlines, so it stays sharp however far anyone zooms in, the file is a fraction of the size, and — the part an image can never do — **the location and phone links stay tappable inside the PDF**. It also matters how you then send it: a chat app re-encodes and downsizes anything sent down its *photo* lane, which is what makes a shared board blurry, so send the PDF (or the JPG) as a **file/document attachment** instead. Ctrl+P gives the same page as the button.
  - Page setup lives in the `PRINT / PDF` section at the bottom of `styles.css`: A4 portrait, and the mission cards are taken out of the masonry back into normal flow, one per row — which is also the right shape for the phone the PDF is mostly read on. Changing to two columns per page is a `column-count` away if you want paper density instead.
