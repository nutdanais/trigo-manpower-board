...
create table if not exists profiles (
  ...
  full_name    text,
  -- the short name the board shows: "Somchai.P". Worked out from the email
  -- address (every TRIGO address is first.last) by fill_display_name() below,
  -- and kept in step with it, but editable — clear it and the automatic name
  -- comes back.
  display_name text,
...
select v.role_key, v.area, v.level from (values
  ...
  -- forward planning (migration-2026-09-24): the Capacity tab and the forecast layer
...
/* An argument that is not given at all leaves that field as it was; an
   argument given as "" clears it. The distinction matters twice:
     ...
     * an older client that calls this with the name alone should not thereby
       wipe the phone number and display name it does not know about. */
...
-- ===== Forward planning (staleness stamps, Capacity, Forecast) =====
-- Also shipped as migration-2026-09-24-forward-planning.sql for a project that
-- already has data; the two are kept identical. See that file's header for why
-- forecasts live in their own tables rather than a status column.
